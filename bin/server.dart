...
Future<Response> _searchHandler(Request request) async {
  ...
  if (query.isEmpty) {
    return Response.ok(jsonEncode({'success': false, 'error': 'Query required'}));
  }
    ...
    if (response.statusCode == 200) {
      ...
      final normalizedResults = results.map((e) {
        return {
          ...
          'artist': _decodeHtml(e['subtitle'] ?? ''), // Saavn puts primary artist in subtitle
          'album': _decodeHtml(e['more_info']?['album'] ?? ''),
          'duration': int.tryParse(e['more_info']?['duration'] ?? '0') ?? 0,
          'coverArt': (e['image'] as String?)?.replaceAll('150x150', '500x500') ?? '',
  ...
  return Response.ok(jsonEncode({'success': true, 'results': []}), headers: {'Content-Type': 'application/json'});
}
...
Future<Response> _streamHandler(Request request) async {
  ...
  if (videoId.isEmpty && query.isEmpty) {
    return Response.ok(jsonEncode({'success': false, 'error': 'id or query required'}));
  }
  ...
  try {
    if (cleanId.isEmpty || videoId.startsWith('search:') || cleanId.length != 11) {
      final searchQuery = query.isNotEmpty ? query : videoId.replaceFirst('search:', '');
      final searchResults = await _yt.search.search(searchQuery);
      if (searchResults.isNotEmpty) {
        cleanId = searchResults.first.id.value;
      } else {
        return Response.ok(jsonEncode({'success': true, 'streams': []}), headers: {'Content-Type': 'application/json'});
      }
    }
